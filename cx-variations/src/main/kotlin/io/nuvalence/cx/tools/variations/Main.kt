...
suspend fun main(args: Array<String>) {
    ...
    OpenAI(token = apiKey).use { client ->
        ...
        val newQuestions = parallelVariations(questionsTab.drop(1).map { it[2] }, gptQuestion, client)
        val newQuestionsTab = newQuestions.zip(questionsTab) { newCell, row ->
            row.toMutableList().apply {
                this[2] = newCell
            }.toList()
        }
        val sheetWriter = SheetWriter(credentialsUrl, spreadsheetId)
        sheetWriter.deleteTab("Questions - Variations")
        sheetWriter.addTab("Questions - Variations")
